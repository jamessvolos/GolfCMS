...
- **The Real Nine** (`#/real`, or "Real hole" in the Round menu): a static
  ...
  by the daily number. Hand-authored from public knowledge (not imagery),
  each georeferenced to where the original lives, regenerated by
  `scripts/author-real-pack.mjs` and held to pack integrity tests. The
  daily is a file: no backend.
- **The photo-ground experiment**: traced rounds alternate between photo and
  painted ground (`src/engine/experiment.js`), `B`-toggle overrides count as
  preference votes, and the Career page reports the verdict once both arms
  have five rounds — the measurement that decides where the next aerial
  effort goes.
